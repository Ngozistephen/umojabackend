...
class ReviewSearchController extends Controller
{
    ...
    public function filter(Request $request)
    {
        ...
        // Handle filtering by product name
        if ($request->has('product_name')) {
            $productName = $request->input('product_name');
            $reviewsQuery->whereHas('product', function ($query) use ($productName) {
                $query->where('name', 'LIKE', "%{$productName}%");
            });
        }

        // Handle filtering by category name
        if ($request->has('category_name')) {
            $categoryName = $request->input('category_name');
            $reviewsQuery->whereHas('product.category', function ($query) use ($categoryName) {
                $query->where('name', 'LIKE', "%{$categoryName}%");
            });
        }

        // Handle filtering by review status
        if ($request->has('review_status')) {
            $reviewStatus = $request->input('review_status');
            $reviewsQuery->where('review_status', 'LIKE', "%{$reviewStatus}%");
        }
